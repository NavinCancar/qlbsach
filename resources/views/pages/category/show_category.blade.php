        ...
        <div class="row">
          ...
          <div class="col-sm-3">
            <ul class="list-group rounded-2 text-center">
              ...
              <li class="list-group-item"><a class="category-list" href="{{ URL::to('/danh-muc-san-pham/tat-ca')}}">- - Tất cả sản phẩm - -</a></li>
              @foreach($category as $key => $cate)    
                <li class="list-group-item 
                <?php
                  if($cate->TLS_MA == $TLS_MA) echo 'bg-active'
                ?>
                "><a class="category-list" href="{{ URL::to('/danh-muc-san-pham/'. $cate->TLS_MA) }}">{{ $cate->TLS_TEN }}</a></li>
              @endforeach
            </ul>
          </div>
          ...
          <!--icon thu tu-->
            <nav aria-label="Page navigation">
                <ul class="pagination justify-content-center text-center pt-4">
                    ...
                    @else
                        <li class="page-item">
                            <a class="page-link" href="{{ $category_by_id->previousPageUrl() }}"><i class="fas fa-angle-left"></i></a>
                        </li>
                    @endif
                    {{-- Pagination Elements --}}
                    @for ($key=0; $key+1<=$category_by_id->lastPage(); $key++)
                            @if ($category_by_id->currentPage() === $key + 1)
                                <li class="page-item active">
                                    <a class="page-link" href="{{ $category_by_id->url($key + 1) }}">{{ $key + 1 }}</a>
                                </li>
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $category_by_id->url($key + 1) }}">{{ $key + 1 }}</a>
                    ...
                    @if ($category_by_id->hasMorePages())
                        <li class="page-item">
                            <a class="page-link" href="{{ $category_by_id->nextPageUrl() }}"><i class="fas fa-angle-right"></i></a>
                        </li>
                    @else

...
@section('admin-content')
                  <div class="row my-2 secondary-bg shadow-sm">
                    ...
                    <h2 class="fs-2 mb-3 p-2 primary-bg text-center primary-text primary-font">{{ $name->TT_TEN }}</h2>
                    @endforeach
                    <!--Header-->
                    ...
                    <div class="table-responsive">
                        <table class="table bg-white rounded shadow-sm table-hover">
                            ...
                            <tbody>
                              ...
                              @foreach($id_status as $key => $all_don)
                    ...
                    <div>
                      <small class="text-muted inline m-t-sm m-b-sm">
                        {{ "Hiển thị ". $id_status->firstItem() ."-". $id_status->lastItem() ." trong tổng số ". $id_status->total() ." dòng dữ liệu" }}
                      </small>
                    </div>
                    <!--icon thu tu-->
                    <nav aria-label="Page navigation">
                        <ul class="pagination pagination-sm m-t-none m-b-none ">
                            ...
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $id_status->previousPageUrl() }}"><i class="fas fa-angle-left"></i></a>
                                </li>
                            @endif
                            {{-- Pagination Elements --}}
                            @for ($key=0; $key+1<=$id_status->lastPage(); $key++)
                                    @if ($id_status->currentPage() === $key + 1)
                                        <li class="page-item active">
                                            <a class="page-link" href="{{ $id_status->url($key + 1) }}">{{ $key + 1 }}</a>
                                        </li>
                                    @else
                                        <li class="page-item">
                                            <a class="page-link" href="{{ $id_status->url($key + 1) }}">{{ $key + 1 }}</a>
                            ...
                            @if ($id_status->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $id_status->nextPageUrl() }}"><i class="fas fa-angle-right"></i></a>
                                </li>
                            @else

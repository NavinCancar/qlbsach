...
\*-----------------------------------*/
    ...
    public function show_employee(){
        ...
        $manager_employee = view('admin.employee.show_employee')->with('edit_employee', $edit_employee)->with('position',$position);
        return view('admin-layout')->with('admin.employee.show_employee', $manager_employee);
    }
    ...
    public function edit_employee($NV_MA){
        ...
        if($NV_MA_get!=$NV_MA&&$CV_MA_get->CV_MA!=1){
            ...
            $manager_employee = view('admin.employee.edit_employee')->with('edit_employee', $edit_employee)->with('position',$position);
            return view('admin-layout')->with('admin.employee.edit_employee', $manager_employee);
        }    
    }
    ...
    public function update_employee(Request $request, $NV_MA){
        ...
        if($get_image){
            $new_image =  'nhanvien'.$NV_MA.'.'.$get_image->getClientOriginalExtension();
...
\*-----------------------------------*/
    ...
    public function add_employee(){
        $this->AuthLoginChu();
        $position = DB::table('CHUC_VU')->orderby('CV_MA')->get(); 
        return view('admin.employee.add_employee')->with('position', $position);
    }
    ...
    public function all_employee(){
        ...
        return view('admin-layout')->with('admin.employee.all_employee', $manager_employee);
    }
    ...
    public function save_employee(Request $request){
        ...
        $get_image= $request->file('NV_DUONGDANANHDAIDIEN');
        if($get_image){
            $new_image =  'nhanvien'.$ma_nv.'.'.$get_image->getClientOriginalExtension();
            $get_image->move('public/backend/images/nhanvien',$new_image);
            $data['NV_DUONGDANANHDAIDIEN'] = $new_image;
    ...
    public function delete_employee($NV_MA){
        ...
        ->leftJoin('lo_xuat','lo_nhap.NV_MA','=','nhan_vien.NV_MA')
        ...
        if($checkforeign1 != 0 || $checkforeign2 != 0){
            Session::put('message','Có lô phụ thuộc nhân viên này, nhân viên này không thể xoá, hãy khoá tài khoản nhân viên');
